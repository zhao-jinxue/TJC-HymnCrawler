...
> 📌 **文档更新说明**：本文件已同步到 **v4 实际状态**。第四阶段（收尾与数据校验）已完成，项目验收归档。
...
### ✅ 第四阶段：收尾与校验（✅ 已完成）
- [x] 编写 `step4_verify_and_report.py` 校验脚本
- [x] 执行数据完整性校验（DB 474 vs 目录 474 vs url_map 474，三方完全一致）
- [x] 执行多媒体资源对账（按版本聚合：五线谱/简谱/鋼琴版 100%，人聲版 461/462，合唱部版 140，四部合唱版 47）
- [x] 失败任务归档（#62 人聲版为服务器端 404，标记归档、不重试）
- [x] 增量更新：沿用"已存在跳过"原则（run_download 已实现）
- [x] 生成 `final_report.txt` 最终统计报告（资源 2070，完整 2069，99%）
- [x] 项目验收与归档（step4 集成进 crawler_fast.py 菜单选项 6）
...
- [x] **`save_to_db` UPSERT 覆盖路径/状态**：Step 2 全量重跑曾清空资源路径与 completed/passed（一度 0/474），已用 CASE 保护空值不覆盖并恢复，全量复验 474/474 健康（2026-08-02）

### 🔄 待优化
- [x] ~~**目录命名清理**~~：第四阶段实测 474 个目录**已无全角/半角空格**（格式 `001_1頌讚獨一真神`），无需处理
- [x] **Step 2 速度**：已用精确等待替代固定 `sleep(0.8)+sleep(0.5)` 与 Tab 点击后 `sleep(0.2)`（extractor.py），仅保留源考面板动画 `sleep(0.3)`；预估每首节省 ~1.3s
- [x] ~~**内存管理**~~：已决定不做（474 首内存约数 MB，无必要，2026-08-02）
- [x] **断点续爬**：extractor.py 新增 progress 文件机制（`Hymn_Downloads/step2_progress.json`，每成功 1 首立即持久化，`clear_progress()`/`resume=False` 全量重跑），已实测二次运行跳过全部已处理
